...
The platform is engineered with a focus on performance, aesthetics, and user experience, reflecting the brand's commitment to "Timeless Sophistication."

### Core Tech Stack
- **Framework:** [Next.js 15 (App Router)](https://nextjs.org/) for optimized performance and routing.
- **UI & Styling:** [Tailwind CSS](https://tailwindcss.com/) for editorial-grade layout and [ShadCN UI](https://ui.shadcn.com/) for consistent, professional components.
- **Motion Design:** [Framer Motion](https://www.framer.com/motion/) for cinematic reveal animations and smooth spatial transitions.
- **Typography:** **EB Garamond** — an elegant, classic serif font that defines the platform's editorial character.
- **State Management:** React Hooks and `useForm` for fluid interactive experiences.
...
### Directory Structure
- `src/app`: Next.js App Router routes and global layouts.
- `src/components/sections`: Individual feature sections (Hero, Portfolio, Quiz, etc.).
- `src/components/ui`: Atomic ShadCN components.
- `src/lib`: Utility functions and placeholder image management.
- `src/hooks`: Custom React hooks for interactive elements.
