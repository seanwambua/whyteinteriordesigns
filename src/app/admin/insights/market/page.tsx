...
import { Globe, Star, ChevronRight, MessageSquare } from "lucide-react";
      ...
      <Card className="rounded-none border-accent/5 bg-white p-10 space-y-8 shadow-xl">
        <div className="flex flex-col md:flex-row items-center justify-between gap-8">
          <div className="flex items-center gap-6">
            <div className="h-14 w-14 bg-amber-500 text-white flex items-center justify-center shadow-xl">
              <Star className="h-6 w-6" />
            </div>
            <div className="space-y-1">
              <h4 className="text-[13px] font-bold uppercase tracking-[0.3em] text-accent">Testimonial Moderation</h4>
              <p className="text-[11px] font-light italic text-muted-foreground uppercase tracking-widest">Curate public client perspectives and authorize brand sentiment vectors</p>
            </div>
          </div>
          <Button asChild variant="outline" className="rounded-none h-14 px-8 border-accent/10 text-accent uppercase tracking-widest text-[10px] font-bold flex gap-3 hover:bg-accent hover:text-white transition-all">
            <Link href="/admin/feedback">Authorize Testimonials <ChevronRight className="h-4 w-4" /></Link>
          </Button>
        </div>
      </Card>
    </div>
