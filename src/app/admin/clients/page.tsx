            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-end pt-6 border-t border-accent/5">
              ...
              <div className="text-right">
                 {client.isArchived ? (
                   ...
                   <Badge className="bg-destructive text-white rounded-none text-[8px] uppercase tracking-widest px-3 py-1 animate-pulse">
                     Dissolution Active
                   </Badge>
                 ) : !client.isActivated ? (
    ...
    <div className="space-y-12 max-w-7xl mx-auto font-body">
      ...
      <Tabs defaultValue="all" className="space-y-8">
        <TabsList className="bg-transparent border-b border-accent/5 w-full justify-start rounded-none h-auto p-0 gap-8">
          ...
          <TabsTrigger value="dissolution" className="rounded-none border-b-2 border-transparent data-[state=active]:border-destructive data-[state=active]:bg-transparent uppercase tracking-[0.3em] text-[10px] font-bold pb-4 px-0 flex gap-2 text-destructive/60 data-[state=active]:text-destructive">
            <ShieldAlert className="h-3.5 w-3.5" /> Dissolution Protocol ({dissolutionProjects.length})
          </TabsTrigger>
        ...
        <TabsContent value="active" className="space-y-6">
          <div className="grid grid-cols-1 gap-6">
            ...
            {activeProjects.length === 0 && (
              <div className="text-center py-20 border border-dashed border-accent/10">
                <p className="text-sm font-light italic text-muted-foreground uppercase tracking-[0.3em]">No financially activated journeys in standard implementation</p>
              </div>
            )}
          </div>
        </TabsContent>
        ...
        <TabsContent value="dissolution" className="space-y-6">
          <div className="grid grid-cols-1 gap-6">
            ...
            {dissolutionProjects.length === 0 && (
              <div className="text-center py-20 border border-dashed border-destructive/10 bg-destructive/[0.02]">
                <p className="text-sm font-light italic text-muted-foreground uppercase tracking-[0.3em]">No commissions currently in dissolution protocol</p>
              </div>
            )}
          </div>
        </TabsContent>
