...
function AddClientForm() {
  ...
  const handleNext = () => {
    if (step === 1 && !formData.accessCode && formData.name) {
      // Auto-generate code for new clients if not already set
    ...
    <div className="max-w-5xl mx-auto space-y-12 font-body pb-24">
            ...
            <AnimatePresence mode="wait">
              {step === 1 && (
                <motion.div key="s1" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-10">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                    <div className="space-y-3"><Label className="text-[12px] font-bold uppercase tracking-widest opacity-60">Full Name</Label><Input placeholder="E.g., Alara Kibaki" className="rounded-none border-accent/20 h-14 text-lg focus:ring-accent transition-none shadow-none" value={formData.name} onChange={(e) => setFormData({...formData, name: e.target.value})} /></div>
                    <div className="space-y-3"><Label className="text-[12px] font-bold uppercase tracking-widest opacity-60">Email Address</Label><Input type="email" placeholder="client@domain.com" className="rounded-none border-accent/20 h-14 text-lg focus:ring-accent transition-none shadow-none" value={formData.email} onChange={(e) => setFormData({...formData, email: e.target.value})} /></div>
                  </div>
                  {formData.accessCode && (
              ...
              {step === 2 && (
                <motion.div key="s2" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-10">
                  ...
                  <div className="space-y-3"><Label className="text-[12px] font-bold uppercase tracking-widest opacity-60">Project Designation</Label><Input placeholder="E.g., Muthaiga Penthouse Renovation" className="rounded-none border-accent/20 h-14 text-xl font-headline italic focus:ring-accent transition-none shadow-none" value={formData.project} onChange={(e) => setFormData({...formData, project: e.target.value})} /></div>
                  <div className="grid grid-cols-1 gap-8">
                    <div className="space-y-3"><Label className="text-[12px] font-bold uppercase tracking-widest opacity-60">Architectural Narrative</Label><Textarea placeholder="Creative requirements..." className="min-h-[120px] rounded-none border-accent/20 text-lg p-6 font-light italic focus:ring-accent transition-none shadow-none" value={formData.description} onChange={(e) => setFormData({...formData, description: e.target.value})} /></div>
                    <div className="space-y-3"><Label className="text-[12px] font-bold uppercase tracking-widest opacity-60 flex items-center gap-2"><LayoutList className="h-3.5 w-3.5" /> Technical Scope of Works</Label><Textarea placeholder="Structural requirements..." className="min-h-[120px] rounded-none border-accent/20 text-base p-6 font-light italic focus:ring-accent bg-secondary/10 transition-none shadow-none" value={formData.workScope} onChange={(e) => setFormData({...formData, workScope: e.target.value})} /></div>
                  </div>
                </motion.div>
              )}
                ...
                <motion.div key="s3" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-10">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                    <div className="space-y-3">
                      ...
                      <Select onValueChange={(v: any) => setFormData({...formData, tier: v})} defaultValue={formData.tier}>
                        <SelectTrigger className="rounded-none border-accent/20 h-14 text-[12px] font-bold uppercase focus:ring-accent transition-none shadow-none"><SelectValue /></SelectTrigger>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-10 pt-6 border-t border-accent/5">
                    <div className="space-y-3">
                      ...
                      <Input type="number" placeholder="E.g., 6" className="rounded-none border-accent/20 h-14 text-2xl font-headline italic focus:ring-accent transition-none shadow-none" value={formData.roomsCount} onChange={(e) => setFormData({...formData, roomsCount: e.target.value})} />
                    </div>
                    <div className="space-y-3">
                      ...
                      <Select onValueChange={(v) => setFormData({...formData, assignedStewardId: v})} value={formData.assignedStewardId}>
                        <SelectTrigger className="rounded-none border-accent/20 h-14 text-[12px] font-bold uppercase focus:ring-accent transition-none shadow-none"><SelectValue placeholder="SELECT STEWARD" /></SelectTrigger>
                ...
                <motion.div key="s4" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-10">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                    <div className="space-y-3">
                      ...
                      <Popover>
                        <PopoverTrigger asChild><Button variant="outline" className="w-full h-14 rounded-none justify-start text-[12px] border-accent/20 uppercase font-bold transition-none shadow-none"><CalendarIcon className="mr-3 h-5 w-5 opacity-40" />{format(formData.startDate, "MMM dd, yyyy")}</Button></PopoverTrigger>
                    ...
                    <div className="space-y-3">
                      ...
                      <Popover>
                        <PopoverTrigger asChild><Button variant="outline" className="w-full h-14 rounded-none justify-start text-[12px] border-accent/20 uppercase font-bold transition-none shadow-none"><CalendarIcon className="mr-3 h-5 w-5 opacity-40" />{format(formData.endDate, "MMM dd, yyyy")}</Button></PopoverTrigger>
                  ...
                  <div className="space-y-6">
                    {formData.milestones.map((m, idx) => (
                      <div key={m.id} className="p-8 border border-accent/5 bg-secondary/5 space-y-6 relative group hover:bg-white hover:shadow-xl transition-none">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                          <div className="space-y-2"><Label className="text-[11px] uppercase font-bold opacity-40">Target Label</Label><Input value={m.label} onChange={(e) => updateMilestone(idx, 'label', e.target.value)} className="rounded-none h-12 text-sm font-bold border-accent/10 focus:ring-accent transition-none shadow-none" placeholder="E.g., Structural Handover" /></div>
                          <div className="space-y-2"><Label className="text-[11px] uppercase font-bold opacity-40">Sync Date</Label><Popover><PopoverTrigger asChild><Button variant="outline" className="w-full h-12 rounded-none justify-start text-[11px] border-accent/10 font-bold transition-none shadow-none"><CalendarIcon className="mr-3 h-4 w-4 opacity-40" />{format(m.date, "MMM dd, yyyy")}</Button></PopoverTrigger><PopoverContent className="w-auto p-0 rounded-none"><Calendar mode="single" selected={m.date} onSelect={(d) => d && updateMilestone(idx, 'date', d)} initialFocus /></PopoverContent></Popover></div>
                        </div>
                      </div>
                    ))}
                  </div>
                    ...
                    {formData.tasks.map((task, idx) => (
                      <div key={task.id} className="p-8 border border-accent/5 bg-secondary/5 space-y-6 relative group hover:bg-white hover:shadow-xl transition-none">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-12 gap-8">
                          <div className="md:col-span-8 space-y-2"><Label className="text-[11px] uppercase font-bold opacity-40">Protocol Identity</Label><Input value={task.title} onChange={(e) => updateTask(idx, 'title', e.target.value)} className="rounded-none h-12 text-sm font-bold border-accent/10 focus:ring-accent transition-none shadow-none" /></div>
                          <div className="md:col-span-4 space-y-2">
                            ...
                            <Select value={task.priority} onValueChange={(v: any) => updateTask(idx, 'priority', v)}>
                              <SelectTrigger className="rounded-none border-accent/10 h-12 text-[11px] font-bold transition-none shadow-none"><SelectValue /></SelectTrigger>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                          <div className="space-y-2">
                            ...
                            <Select value={alloc.vendorName} onValueChange={(v) => updateAllocation(idx, 'vendorName', v)}>
                              <SelectTrigger className="rounded-none h-12 text-[12px] font-bold border-accent/10 focus:ring-accent transition-none shadow-none"><SelectValue placeholder="SELECT FROM REGISTRY" /></SelectTrigger>
                          ...
                          <div className="space-y-2"><Label className="text-[11px] uppercase font-bold opacity-40">Professional Role</Label><Input value={alloc.role} onChange={(e) => updateAllocation(idx, 'role', e.target.value)} className="rounded-none h-12 text-sm font-bold border-accent/10 focus:ring-accent transition-none shadow-none" /></div>
                        </div>
                      </div>
                    ))}
                  </div>
      ...
      <AlertDialog open={isConfirmOpen} onOpenChange={setIsConfirmOpen}>
        <AlertDialogContent className="rounded-none border-accent/20 font-body p-10 bg-white shadow-2xl">
          ...
          <AlertDialogFooter className="pt-10"><AlertDialogCancel className="rounded-none uppercase tracking-widest text-[12px] font-bold h-14 px-8 border-accent/10 transition-none shadow-none">Abort</AlertDialogCancel><AlertDialogAction onClick={executeFinalSubmit} className="bg-accent text-white rounded-none uppercase tracking-widest text-[12px] font-bold h-14 px-10 shadow-xl transition-none border-none">Authorize Dossier</AlertDialogAction></AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
    </div>
  );
