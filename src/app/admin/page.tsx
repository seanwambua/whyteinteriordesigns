      ...
      >
        <div>
          <div className="flex items-center gap-4 mb-2">
            ...
            <span className="text-accent text-[12px] font-bold uppercase tracking-[0.4em]">Studio Intelligence</span>
          </div>
        ...
        <AlertDialog>
          <AlertDialogTrigger asChild>
            <Button variant="outline" className="rounded-none border-destructive/20 text-destructive hover:bg-destructive hover:text-white h-12 uppercase tracking-widest text-[11px] font-bold flex gap-2">
          ...
          <AlertDialogContent className="rounded-none border-accent/20 font-body">
            <AlertDialogHeader className="space-y-4">
              <div className="flex items-center gap-3">
                ...
                <span className="text-destructive text-[11px] font-bold uppercase tracking-[0.3em]">Critical Protocol</span>
              </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
                ...
                {inquiries.slice(0, 3).map((inquiry) => (
                  <div key={inquiry.id} className="flex items-center justify-between p-6 border border-accent/5 hover:border-accent/10 transition-all bg-secondary/10 group cursor-pointer">
                    <div className="space-y-1">
                      <p className="text-sm font-bold uppercase tracking-[0.2em] group-hover:text-accent transition-colors">{inquiry.name}</p>
                ...
                {archivedJourneys.slice(0, 4).map((p) => (
                  <Link key={p.id} href={`/admin/clients/${p.id}`} className="group p-6 border border-accent/5 hover:border-accent/20 bg-secondary/5 flex flex-col justify-between h-32 transition-all">
                    <div className="flex justify-between items-start">
                      ...
                      <Badge className="bg-black text-white rounded-none text-[8px] uppercase tracking-widest px-1.5 py-0">Retired</Badge>
                    </div>
                    <h4 className="text-base font-headline italic text-accent/80 group-hover:text-accent transition-colors">{p.project}</h4>
        ...
        <div className="space-y-8">
            ...
            <div className="space-y-8">
              {feedback.filter(fb => fb.isApproved).slice(0, 1).map(fb => (
                <div key={fb.id} className="p-6 bg-accent/5 border-l-2 border-accent italic">
                  <p className="text-sm font-light text-accent/80 leading-relaxed mb-4">
                    "{fb.comment}"
                  </p>
          ...
          <Card className="rounded-none border-accent/10 shadow-xl bg-secondary/10 p-8 border-dashed">
            <h4 className="text-[11px] uppercase tracking-[0.4em] font-bold text-accent/40 mb-4">System Log</h4>
            <ul className="space-y-3">
              <li className="text-[10px] uppercase tracking-widest text-muted-foreground flex items-center gap-2">
